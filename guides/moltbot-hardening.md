# Moltbot 10-Minute Security Hardening Checklist

**Reduce your security risk by 80% in 10 minutes**

**Target Audience:** Anyone running Moltbot/Clawdbot on VPS, Mac mini, or local machine  
**Time Required:** 10 minutes  
**Difficulty:** Beginner-friendly  
**Prerequisites:** SSH access to your Moltbot server
...
**What you'll need:**
- SSH access to server running Moltbot
...
**Current risk:** Your Moltbot gateway is probably exposed to the entire internet.
...
chmod 600 ~/.clawdbot/.env 2>/dev/null || true
```

**Verify:**
```bash
ls -la ~/.clawdbot/ | head -5
...
**Option A: Quick Fix (Moltbot UI)**

1. Open Moltbot UI (via SSH tunnel from Step 1)
...
```bash
# Download scanner
curl -O https://raw.githubusercontent.com/CyberStrategyInstitute/ai-safe2-framework/main/examples/moltbot/scanner.py
...
Send your Moltbot this message:
```
...
**If bot tries to execute instead:** Check that the file is in the right location and Moltbot restarted.
...
1. **Deploy Control Gateway** (30 minutes)
   ...
   - [Gateway Setup Guide](../examples/moltbot/gateway/README.md)
...
0 9 * * 1 python3 ~/scanner.py --path ~/.clawdbot --output ~/moltbot-scan-$(date +\%Y\%m\%d).txt
```
...
#!/bin/bash
DATE=$(date +%Y%m%d)
tar -czf ~/backups/moltbot-$DATE.tar.gz ~/.clawdbot/
find ~/backups/ -name "moltbot-*.tar.gz" -mtime +30 -delete
EOF
...
tail -100 ~/.clawdbot/logs/audit.log | grep -E "BLOCK|CRITICAL|injection"
```
...
7. **Stay updated**
   - Run scanner monthly
   - Update Moltbot when patches released
...
8. **If going to production:**
   ...
   - [Enterprise Guide](./moltbot-enterprise.md)
...
- [ ] Update Moltbot if patches available
...
In Moltbot UI → Settings → Confirmations:
...
**Ideal setup:**
- Moltbot on dedicated VPS or Mac mini
- Your personal laptop NEVER runs Moltbot
- Sensitive files (taxes, passwords, etc.) NOT accessible to Moltbot
...
### Q: Can I run Moltbot on my primary laptop?
...
**Problem:** Can't access Moltbot UI after hardening
...
**More help:** [Full Troubleshooting Guide](../examples/moltbot/troubleshooting.md)
...
- [Moltbot Examples](../examples/moltbot/)
- [Control Gateway Setup](../examples/moltbot/gateway/)

**Tools:**
- [Security Scanner](../examples/moltbot/scanner.py)
- [Memory Protocol](../examples/moltbot/moltbot_memory.md)
- [Troubleshooting Guide](../examples/moltbot/troubleshooting.md)
...
- [Blog Posts](https://cyberstrategyinstitute.com/tag/moltbot/)
...
**Done? You're now 80% more secure than most Moltbot users. Great work!** 🎉
