# LangGraph Sovereign Runtime — 15-Minute Quickstart
...
- Python 3.9+
- pip
- LangGraph project (existing or new)
...
cd examples/langgraph-sovereign-runtime
...
handler = SovereignStateGraph(act_tier=ACTTier.ACT3)

# Works with any LangGraph chain, agent, or LCEL expression
result = chain.invoke(
    {"input": user_input},
...
```python
from enforcement import SovereignLangGraph, ACTTier

sovereign = SovereignLangGraph(
...
```bash
cp ci-cd/github-actions-langgraph-safe.yml \
