...
# Legacy sys.path hack from when this test lived at examples/mcp-security-toolkit/tests/;
# harmless no-op now that aisafe2_mcp_tools is a properly installed top-level package
# (safe2 CLI consolidation), kept so this file still runs standalone outside the venv too.
sys.path.insert(0, str(Path(__file__).parent.parent / "src"))

import aisafe2_mcp_tools.scan
from aisafe2_mcp_tools.scan.analyzer import Finding, MCPScanner
...
class TestInjectionPatternCoverage:
    ...
    def test_pattern_detected(self, text: str, expected_family: str):
        ...
        assert expected_family in families, (
            f"Expected '{expected_family}' not detected in: {text!r}\n"
...
class TestSanitizeValue:
    ...
    def test_list_structure_preserved(self):
        ...
        result, _findings = sanitize_value(items)
    ...
    def test_deeply_nested_sanitization(self):
        ...
        result, _findings = sanitize_value(nested)
    ...
    def test_multiple_patterns_all_redacted(self):
        ...
        result, _findings = sanitize_value(text)
...
class TestToolInjectionAnalysis:
    ...
    def test_poisoned_tool_description_detected(self):
        ...
        inj_check, _fsp_check, _ssrf_check, _session_check = assessor._analyze_tools(tools_data)
    ...
    def test_empty_tool_list_passes(self):
        ...
        inj_check, fsp_check, ssrf_check, _session_check = assessor._analyze_tools(tools_data)
        assert inj_check.passed
...
class TestMCP11SchemaTemporalProfiling:
    ...
    def test_fix_template_exists(self):
        ...
        fixes_dir = Path(aisafe2_mcp_tools.scan.__file__).parent / "fixes"  # relocated: safe2 CLI consolidation
