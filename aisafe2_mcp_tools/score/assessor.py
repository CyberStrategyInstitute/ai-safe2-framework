...
def to_dict(self: CheckResult) -> dict[str, Any]:
        return {
...
class MCPAssessor:
    ...
    _SECURITY_HEADERS: ClassVar[list[tuple[str, str]]] = [
    ...
    async def assess(self) -> ScoreReport:
        ...
        async with httpx.AsyncClient(
            ...
            if tools_result is not None:
                ...
                checks.extend(self._analyze_tools(tools_data))
            else:
                errors.append(
    ...
    async def _check_tls(self, client: httpx.AsyncClient) -> CheckResult:
        ...
        for probe_url in (f"{self.server_url}/health", self.server_url):
            try:
                ...
                log.debug("tls.probe_failed", url=probe_url, error=type(exc).__name__)
                continue
    ...
    async def _check_rate_limiting(self, client: httpx.AsyncClient) -> CheckResult:
        ...
        for headers in (self._auth_headers, self._unauth_headers):
            try:
                ...
                log.debug("rate.probe_failed", error=type(exc).__name__)
                continue
    ...
    def _analyze_tools(self, tools_data: dict[str, Any]) -> list[CheckResult]:
        ...
        inj_findings: list[dict[str, Any]] = []
        inj_score = 20
        for tool in tools:
