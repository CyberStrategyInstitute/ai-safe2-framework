...
def sanitize_value(
    value: object, field_path: str = "root"
    ...
    if isinstance(value, dict):
        ...
        for k, v in value.items():
            sanitized_v, nested_findings = sanitize_value(v, f"{field_path}.{k}")
    ...
    if isinstance(value, list):
        ...
        for i, item in enumerate(value):
            sanitized_item, nested_findings = sanitize_value(item, f"{field_path}[{i}]")
