...
class ISO42001Report:
    ...
    def generate_report(
        ...
        if result.act_estimate:
            ...
            print(
                f"   ACT Tier: {act.get('estimated_tier', 'unknown')} "
                f"({act.get('confidence', '?')} confidence)"
            )
            if act.get("hear_required"):
                print(
                    "   WARNING: HEAR Required: Designate a Human Ethical Agent of Record before deployment"
                )
            if act.get("cp9_required"):
                print("   WARNING: CP.9 Required: Agent Replication Governance must be implemented")
    ...
    def _build_artifact(self, result: ScanResult) -> dict[str, Any]:
        ...
        for fw_id, fw_name in FRAMEWORK_NAMES.items():
            ...
            fallback_hits = [
                c for c in fallback_controls if any(fc.startswith(c) for fc in failed_controls)
            ]
        ...
        for pid in ("P1", "P2", "P3", "P4", "P5", "CP"):
            ...
            pillar_breakdown[pid] = {
                ...
                "critical": crit,
                "high": high,
                "controls_failed": sorted({v.control_id for v in pillar_vs}),
            }
        ...
        artifact = {
            ...
            "compliance_summary": {
                "frameworks_passing": sum(
                    1 for f in framework_status.values() if f["status"] == "PASS"
                ),
                "frameworks_failing": sum(
                    1 for f in framework_status.values() if f["status"] == "FAIL"
                ),
        ...
        results_sarif = [
            {
                ...
                "message": {
                    "text": f"[{v.control_id}] {v.description} | Fix: {v.remediation[:120]}"
                },
                "locations": [
                    {
                        "physicalLocation": {
