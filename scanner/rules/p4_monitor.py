...
def _check_bedrock_unmonitored(
    ...
    high_risk_bedrock = [
        ...
        r"update_guardrail",
        r"UpdateDataSource",
        r"update_data_source",
        r"UpdateKnowledgeBase",
        r"update_knowledge_base",
        r"DeleteGuardrail",
        r"delete_guardrail",
        r"bedrock_agent\.update",
        r"bedrock-agent.*update",
    ]
    monitoring_words = {
        "monitor",
        "alert",
...
def _check_tool_invocation_without_baseline(
    content: str, lines: list[str], filepath: str
) -> list[tuple[int, str]]:
    """
        ...
        for pat in high_impact_patterns:
            if re.search(pat, line, re.IGNORECASE):
                ...
                if not any(w in context for w in hitl_words):
                    findings.append(
                        (i + 1, f"High-impact action without HITL checkpoint: {line.strip()[:60]}")
                    )
                break
    return findings


def _check_missing_security_logging(
    content: str, lines: list[str], filepath: str
) -> list[tuple[int, str]]:
    """Only assess logging when executable agent or tool-dispatch behavior is present."""
    lower = content.lower()
    executable = any(
        token in lower
        for token in ("@mcp.tool", "tools/call", "call_tool(", "agent.run(", "llm.invoke(")
    )
    logging = any(
        token in lower
        for token in (
    ...
    if executable and not logging:
        return [(1, "Executable agent/tool path has no visible security-event forwarding")]
    return []
...
P4_RULES: list[Rule] = [
    ...
    Rule(
        control_id="P4.T8.3",
        severity="HIGH",
        description="No security-event forwarding found for an executable agent or tool path.",
        remediation="Forward authentication, tool invocation, and anomaly events to a centralized, tamper-evident store.",
        check_fn=_check_missing_security_logging,
        file_exts=(".py", ".js", ".ts"),
    ),
    Rule(
        control_id="P4.T8.5",
