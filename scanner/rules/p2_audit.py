...
def _check_llm_call_without_logging(
        ...
        for pat in llm_call_patterns:
            if re.search(pat, line, re.IGNORECASE):
                ...
                if not any(w in context for w in log_indicators):
                    findings.append(
                        (i + 1, f"LLM API call without logging context: {line.strip()[:60]}")
                    )
                break
    return findings


def _check_missing_owner_of_record(
    content: str, lines: list[str], filepath: str
) -> list[tuple[int, str]]:
    """
...
def _check_rag_corpus_without_tracking(
    content: str, lines: list[str], filepath: str
) -> list[tuple[int, str]]:
    """
...
P2_RULES: list[Rule] = [
    ...
    Rule(
        ...
        pattern=r"(torch\.load|joblib\.load|pickle\.load|from_pretrained)",
        file_exts=(".py",),
    ),
    # A2.6 — RAG Corpus Diff Tracking
    Rule(
