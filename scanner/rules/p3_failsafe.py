...
def _check_recursion_without_limit(
            ...
            if re.search(pat, line, re.IGNORECASE):
                ...
                if not any(w in context for w in limit_words):
                    ...
                    if "while true" in line.lower():
                        ...
                        findings.append(
                            (i + 1, f"Tool-calling loop without depth limit: {line.strip()[:60]}")
                        )
                    break
    return findings
...
P3_RULES: list[Rule] = [
    ...
    Rule(
        ...
        pattern=r"(AgentPipeline|SequentialAgent|orchestrator\.(run|invoke)|pipeline\.execute)",
        file_exts=(".py", ".js", ".ts", ".yaml"),
    ),
    # Recovery checks
    Rule(
