...
def _check_memory_write_governance(
    content: str, lines: list[str], filepath: str
) -> list[tuple[int, str]]:
    """
        ...
        for pat in write_patterns:
            if re.search(pat, line, re.IGNORECASE):
                ...
                if not any(g in context for g in governance_words):
                    findings.append(
                        (i + 1, f"Memory write without governance wrapper: {line.strip()[:60]}")
                    )
    return findings


def _check_n8n_expression_injection(
    content: str, lines: list[str], filepath: str
) -> list[tuple[int, str]]:
    """
    ...
    ai_node_markers = [
        r'"type":\s*"@n8n/n8n-nodes-langchain',
        r'"type":\s*"n8n-nodes-base.openAi',
        r'"type":\s*"@n8n/n8n-nodes-base.agent',
        r'"nodeType".*[Aa]i',
        r'"nodeType".*[Ll][Ll][Mm]',
    ]
...
def _check_pickle_model_loading(
    content: str, lines: list[str], filepath: str
) -> list[tuple[int, str]]:
    """
