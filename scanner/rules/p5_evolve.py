...
VULNERABLE_VERSIONS = [
    ...
    (
        "langchain-core",
        r"langchain-core[=<>!~]+0\.(1)\.",
        "LangChain Core <0.2 — prompt injection exposure",
    ),
    # transformers
    (
        "transformers",
        r"transformers[=<>!~]+4\.(2[0-9]|[0-1][0-9])\.",
        "Hugging Face Transformers <4.30 — deserialization vulnerability",
    ),
    # openai python client
    (
        "openai",
        r"openai[=<>!~]+0\.",
        "OpenAI Python SDK v0.x — deprecated, security updates stopped",
    ),
    ...
    (
        "requests",
        r"requests[=<>!~]+2\.([01][0-9])\.",
        "requests <2.20 — SSRF and redirect vulnerability",
    ),
    # pydantic v1 with known issues
    (
        "pydantic",
        r"pydantic[=<>!~]+1\.(0|1|2|3|4|5|6|7|8)\.",
        "Pydantic <1.9 — validation bypass vulnerability",
    ),
]
        ...
        if re.search(vuln_pattern, content, re.IGNORECASE):
            for i, line in enumerate(lines):
                if re.search(vuln_pattern, line, re.IGNORECASE):
                    findings.append(
                        (i + 1, f"Vulnerable dependency: {description} — {line.strip()[:60]}")
                    )
    return findings


def _check_missing_adversarial_eval(
    content: str, lines: list[str], filepath: str
) -> list[tuple[int, str]]:
    """
    ...
    if not has_eval:
        findings.append(
            (1, "CI/CD pipeline detected without adversarial evaluation gate (E5.1 required)")
        )
    return findings


def _check_missing_threat_intelligence(
    content: str, lines: list[str], filepath: str
) -> list[tuple[int, str]]:
    lower = content.lower()
    uses_ai = any(re.search(pattern, content, re.MULTILINE) for pattern in AI_FRAMEWORK_IMPORTS)
    has_feed = any(
        token in lower for token in ("mitre", "atlas", "cve", "nvd", "osv", "threat_intel")
    )
    return (
        []
        if not uses_ai or has_feed
        else [(1, "AI framework use has no threat-intelligence reference")]
    )


def _check_missing_red_team_retention(
    content: str, lines: list[str], filepath: str
) -> list[tuple[int, str]]:
    lower = content.lower()
    is_ci = filepath.endswith((".yml", ".yaml")) and "jobs:" in lower
    has_retention = any(
        token in lower for token in ("red-team", "red_team", "adversarial", "artifact")
    )
    return (
        []
        if not is_ci or has_retention
        else [(1, "CI workflow has no red-team artifact retention")]
    )
...
P5_RULES: list[Rule] = [
    ...
    Rule(
        ...
        check_fn=_check_missing_red_team_retention,
        file_exts=(".yaml", ".yml"),
    ),
]
